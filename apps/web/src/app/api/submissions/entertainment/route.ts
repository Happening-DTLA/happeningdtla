...
import { EntertainmentSubmissionBody, createEntertainmentSubmission } from "@/lib/participation-submissions";
...
export async function POST(request: Request) {
  const limited = await enforceRateLimit("entertainment-submission:ip", clientIp(request), SUBMIT_RULES);
  ...
  const parsed = EntertainmentSubmissionBody.safeParse(raw);
  if (!parsed.success) {
  ...
  const submission = await createEntertainmentSubmission(parsed.data);
  return ok({ id: submission.id, status: submission.status, actName: submission.actName, });
}
